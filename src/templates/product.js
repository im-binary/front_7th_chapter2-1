import { Header, PageLayout } from "../components/index.js";

const defaultBreadcrumb = /*html*/ `<button data-breadcrumb="reset" class="text-xs hover:text-blue-800 hover:underline">전체</button>`;

const createProductFilter = ({ breadcrumbMarkup, categoriesMarkup } = {}) => /*html*/ `
		...
		<div class="space-y-3">
			<div class="space-y-2">
				<div class="flex items-center gap-2">
					...
					${breadcrumbMarkup ?? defaultBreadcrumb}
				</div>
				${categoriesMarkup ?? ""}
			</div>
...
const categoryLoadingMarkup = /*html*/ `
	<div class="flex flex-wrap gap-2">
		<div class="text-sm text-gray-500 italic">카테고리 로딩 중...</div>
	</div>
`;

const categoryRootMarkup = /*html*/ `
	<div class="flex flex-wrap gap-2">
...
const categorySecondLevelMarkup = /*html*/ `
	<div class="space-y-2">
		<div class="flex flex-wrap gap-2">
...
const categorySecondLevelActiveMarkup = /*html*/ `
	<div class="space-y-2">
		<div class="flex flex-wrap gap-2">
			<button data-category1="생활/건강" data-category2="생활용품" class="category2-filter-btn text-left px-3 py-2 text-sm rounded-md border transition-colors bg-white border-gray-300 text-gray-700 hover:bg-gray-50">
				생활용품
...
const categoryOneBreadcrumb = /*html*/ `<button data-breadcrumb="reset" class="text-xs hover:text-blue-800 hover:underline">전체</button><span class="text-xs text-gray-500">&gt;</span><button data-breadcrumb="category1" data-category1="생활/건강" class="text-xs hover:text-blue-800 hover:underline">생활/건강</button>`;
const categoryTwoBreadcrumb = /*html*/ `<button data-breadcrumb="reset" class="text-xs hover:text-blue-800 hover:underline">전체</button><span class="text-xs text-gray-500">&gt;</span><button data-breadcrumb="category1" data-category1="생활/건강" class="text-xs hover:text-blue-800 hover:underline">생활/건강</button><span class="text-xs text-gray-500">&gt;</span><span class="text-xs text-gray-600 cursor-default">주방용품</span>`;

const productLoadingContent = /*html*/ `
	${createProductFilter({ categoriesMarkup: categoryLoadingMarkup })}
	<div class="mb-6">
		<div>
...
const productLoadedContent = /*html*/ `
	${createProductFilter({ categoriesMarkup: categoryRootMarkup })}
	<div class="mb-6">
		<div>
...
export const 상품목록_레이아웃_로딩 = PageLayout({
  children: productLoadingContent,
});

export const 상품목록_레이아웃_로딩완료 = PageLayout({
  header: Header({ cartCount: 4 }),
  children: productLoadedContent,
});

export const 상품목록_레이아웃_카테고리_1Depth = PageLayout({
  children: createProductFilter({
    breadcrumbMarkup: categoryOneBreadcrumb,
    categoriesMarkup: categorySecondLevelMarkup,
  }),
});

export const 상품목록_레이아웃_카테고리_2Depth = PageLayout({
  children: createProductFilter({
    breadcrumbMarkup: categoryTwoBreadcrumb,
    categoriesMarkup: categorySecondLevelActiveMarkup,
  }),
});
